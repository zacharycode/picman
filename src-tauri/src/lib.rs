...
fn cache_hash(root: &Path, source: &ThumbnailSource, quality: &str, metadata: &fs::Metadata) -> String {
  let mut hasher = Sha256::new();
...
fn scan_library_folder(root_path: String) -> Result<ScanLibraryResponse, String> {
    ...
    .filter_map(Result::ok)
  {
    ...
    let path = entry.path();
    let Some(kind) = get_kind(path) else {
      continue;
    };
    ...
    assets.push(ScannedAsset {
      ...
      preview_url: Some(tauri::Url::from_file_path(path).map_err(|_| "无法转换图片路径".to_string())?.to_string()),
      relative_path: normalized_relative_path,
      size_kb: std::cmp::max(1, metadata.len().div_ceil(1024)),
      source_path: normalize_path(path),
      swatch: swatch_for(assets.len()).to_string(),
      tags: Vec::new(),
...
fn generate_thumbnail(
  ...
  let metadata = fs::metadata(&source_path).map_err(|error| format!("无法读取源文件：{error}"))?;
  let preset = thumbnail_preset(&quality);
  let hash = cache_hash(&root, &source, &quality, &metadata);
  let cache_dir = root.join(".picman").join("cache").join("thumbnails").join(&quality);
      ...
      .unwrap_or(false)
  {
    ...
    fs::copy(&source_path, &output_path).map_err(|error| format!("无法写入 SVG 缩略图缓存：{error}"))?;
    let output_metadata =
  ...
  if let Some((extension, format)) = passthrough {
    ...
    fs::copy(&source_path, &output_path).map_err(|error| format!("无法写入缩略图缓存：{error}"))?;
    let output_metadata =
      fs::metadata(&output_path).map_err(|error| format!("无法读取缩略图缓存：{error}"))?;
